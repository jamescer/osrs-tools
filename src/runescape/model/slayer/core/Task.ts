...
export interface TaskOptions {
  ...
  requirements: Requirement[];
  extended?: { min: number; max: number } | null;
  weight?: number;
  alternatives?: string[];
  slayerExp?: number;
  combatLevels?: number[];
...
class Task {
  ...
  static create(options: TaskOptions): Task {
    // Use default weight and slayer exp if not provided
    const defaultWeight = 8;
    const defaultSlayerExp = 0;

    return new Task(
      options.name,
      options.quantity.min,
      options.quantity.max,
      options.requirements,
      options.extended?.min ?? null,
      options.extended?.max ?? null,
      options.alternatives || [],
      options.weight ?? defaultWeight,
      options.combatLevels || [],
      options.slayerExp ?? defaultSlayerExp,
      (options.locations || []) as string[],
      options.wildernessLevels || [],
      (options.bosses || []) as string[],
    );
  }
