...
import { SlayerLevelRequirement } from 'src/runescape/model/Requirement';
import { SlayerMaster } from 'src/runescape/model/slayer/SlayerMaster';
...
describe('SlayerMaster - Slayer master configuration and task assignment', () => {
  ...
  const taskPoints = { 1: 2, 10: 10, 100: 100, 50: 50 };
  const eliteDiaryTaskPoints = { 1: 3, 10: 15, 100: 150, 50: 75 };
  ...
  describe('Real-world slayer masters', () => {
    it('should represent Turael (lowest level master)', () => {
      const turaelTasks = [new Task('Spiders', 10, 20, []), new Task('Goblins', 8, 15, [])];
      const turael = new SlayerMaster('Turael', turaelTasks, 'Burthorpe', 1, 'https://wiki.osrs.wiki/Turael');
    ...
    it('should represent Duradel (high level master)', () => {
      ...
      const duradel = new SlayerMaster('Duradel', duadelTasks, 'Shilo Village', 50, 'https://wiki.osrs.wiki/Duradel');
