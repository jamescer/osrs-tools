...
describe('SlayerUnlock', () => {
    ...
    it('should contain specific SlayerUnlocks with correct properties', () => {
        const gargoyleSmasher = SlayerUnlocks.find(unlock => unlock.name === 'Gargoyle smasher');
        expect(gargoyleSmasher).toBeDefined();
        expect(gargoyleSmasher?.cost).toBe(120);
        expect(gargoyleSmasher?.description).toBe('Gargoyles are automatically dealt the finishing blow if the player has a Rock hammer in their inventory.');
        const likeABoss = SlayerUnlocks.find(unlock => unlock.name === 'Like a boss');
        expect(likeABoss).toBeDefined();
        expect(likeABoss?.cost).toBe(200);
        expect(likeABoss?.description).toBe('Konar, Duradel, Krystilia and Nieve will be able to assign boss monsters as your task, excluding the Corporeal Beast.');
    });
