...
async function fetchQuestDetails(quest: Quest): Promise<DetailedQuest | null> {
  ...
  try {
    ...
    const leadDevelopers = $('th:contains("Lead developer(s)")')
      .next()
      .text()
      .trim();
    ...
    $("li.scp").each((i, elem) => {
      ...
      skillRequirements.push({ boostable: !isBoostable, level, skill });
    });
    ...
    return {
      ...
      leadDevelopers,
      membersOnly,
      quickGuideLink: quickGuideLink
        ? `https://oldschool.runescape.wiki${quickGuideLink}`
        : undefined,
      skillRequirements,
      speedrunGuideLink: speedrunGuideLink
        ? `https://oldschool.runescape.wiki${speedrunGuideLink}`
        : undefined,
    };
  } catch (error) {
...
async function scrapeQuestDetails() {
  ...
  fs.writeFileSync(
    "tools/DetailedQuests.json",
    JSON.stringify(detailedQuests, null, 2),
  );
